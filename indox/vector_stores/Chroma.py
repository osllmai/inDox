...
#
# from indox.core import VectorStore
# import chromadb
...
#     """
#     A concrete implementation of VectorStore using Chroma for storage.
#
#     Attributes:
#         db (Chroma): The Chroma vector store.
#     """
...
#         self.db = self.client.create_collection(name=collection_name,embedding_function=self.embeddings)
#
#     def add_document(self, docs: List[str]):
#         """
#         Adds documents to the Chroma vector store.
#
#         Args:
...
#         """
#         try:
#             # Assuming docs are plain text if not Document instances
#             self.db.add(documents=docs)
#             logger.info("Documents added successfully to the vector store.")
#         except Exception as e:
#             logger.error(f"Failed to add document: {e}")
#             raise RuntimeError(f"Can't add document to the vector store: {e}")
#
...
#         """
#         try:
#             retrieved = self.db.query(query_texts=query, n_results=top_k)
#             # context = [d[0].page_content for d in retrieved]
#             # scores = [d[1] for d in retrieved]
#             # return context, scores
#             print(retrieved)
