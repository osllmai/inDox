...
class YOLOv6Model:
    def __init__(self, weights_url="https://github.com/meituan/YOLOv6/releases/download/0.4.0/yolov6s.pt", device="cuda"):
        """
        Initialize the YOLOv6 model and download weights if necessary.
        """
        self.device = torch.device(device if torch.cuda.is_available() else "cpu")
        self.weights_path = "yolov6s.pt"
    ...
    def detect_objects(self, image_path, threshold=0.25):
        ...
        command = f"python tools/infer.py --weights {self.weights_path} --source {image_path} --save-dir {output_dir} --conf-thres {threshold}"
